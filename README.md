...
- Copy `.env.example` -> `.env` (`just init`) and set the three private key passwords and the cert subject values. That is all `.env` holds - the computed paths live in `paths.sh`, which is tracked in git, so a change to the layout reaches every clone instead of only new ones. Scripts source `.env` then `paths.sh`.
...
**`just revoke` records a revocation; it does not enforce one.** It marks the certificate revoked in the CA database and regenerates `output/intermediate/crl/intermediate.crl.pem`, but nothing here publishes that CRL and issued certificates carry no `crlDistributionPoints` extension. No client will ever fetch or check it, so a revoked certificate stays trusted everywhere until it expires.

Publishing a CRL properly means an HTTP endpoint, a renewal cadence and clients that bother to check - a lot of machinery for internal services. The practical remedy is to replace the certificate:

```sh
just renew NAME
```

then deploy the new key and fullchain to the host and restart the service. `just renew` is safe to run repeatedly, including on a certificate that is already revoked.
...
`output/` is the whole CA: both private keys, the issuing database, and the serial and revocation state. It is gitignored, deliberately, and nothing here backs it up. Lose it and you cannot issue or revoke anything, and re-establishing trust means importing a new root by hand on every device and browser profile you own.

Keep an encrypted copy somewhere. It is small:

```sh
tar czf - output | age -p > local-ca-backup.tar.gz.age   # or gpg -c, or whatever you already use
```
...
The same hooks run in CI (`.github/workflows/lint.yml`), so a clone that never ran `just setup` still cannot push whitespace damage or a stray key.

`.python-version` pins the interpreter for `pre-commit` and `commitizen` only - there is no Python in this repo.
